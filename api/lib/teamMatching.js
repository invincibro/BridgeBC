const { normalizeMultiValueField } = require("./volunteerSimilarity");

function uniqueValues(values) {
  return [...new Set((values || []).filter(Boolean))];
}

function combineTeamFields(teamMembers, fieldName) {
  return uniqueValues(
...
function computeCoverageScore(teamValues, orgValues) {
...
function computeAvailabilityCoverage(teamMembers, orgAvailability) {
...
function computeBackgroundCoverage(teamMembers, orgRequirement) {
  if (!orgRequirement) {
    return 1;
  }
...
function generateTeamReasons(teamMembers, org) {
  const reasons = [];
  ...
  if (sharedLanguages.length > 0) {
    reasons.push(`Shared ${sharedLanguages[0][0].toUpperCase()}${sharedLanguages[0].slice(1)} support improves language coverage`);
  }

  const sector = normalizeMultiValueField(org?.sector)[0];
  if (sector && combinedCauses.includes(sector)) {
  ...
  if (reasons.length === 0) {
    reasons.push("This team improves overall coverage for the organization.");
  }
...
function recommendVolunteerTeam(currentVolunteer, featuredOrganization, allVolunteers, similarVolunteers) {
  if (!currentVolunteer || !featuredOrganization) {
    return null;
  }

  const candidatePool = (
    similarVolunteers && similarVolunteers.length > 0 ? similarVolunteers : allVolunteers || []
  ).slice(0, 3);
