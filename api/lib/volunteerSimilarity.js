...
function computeArrayOverlapScore(a, b) {
...
function computeAvailabilitySimilarity(a, b) {
  const left = normalizeMultiValueField(a);
  const right = normalizeMultiValueField(b);
  ...
  const leftSet = new Set(left);
  if (right.some((item) => leftSet.has(item))) {
    return 1;
  }
  ...
  const similarOverlap = leftParsed.some((leftToken) =>
    rightParsed.some((rightToken) => {
      ...
      const sameDayGroup =
        (leftToken.weekday && rightToken.weekday) ||
        (leftToken.weekend && rightToken.weekend);
      const sameTimeGroup =
...
function computeNumericCloseness(a, b, maxRange) {
  const left = Number(a);
...
function computeBooleanMatch(a, b) {
...
function normalizeExperience(value) {
  ...
  if (normalized.includes("extensive") || normalized.includes("experienced") || normalized.includes("3+")) return 3;
  if (normalized.includes("moderate")) return 2;
  if (normalized.includes("some") || normalized.includes("1-2")) return 1;
  return 0;
}
...
function getSharedNonEnglishLanguages(firstVolunteer, secondVolunteer) {
  const first = normalizeMultiValueField(firstVolunteer.languages_spoken).filter((language) => language !== "english");
  const second = normalizeMultiValueField(secondVolunteer.languages_spoken).filter((language) => language !== "english");
...
function collectCommonFactors(firstVolunteer, secondVolunteer, detailScores) {
  ...
  if (sharedCauses[0]) factors.push({ weight: detailScores.causeAreas, label: `Shared cause: ${titleCase(sharedCauses[0])}` });
  if (sharedSkills[0]) factors.push({ weight: detailScores.skills, label: `Shared skill: ${titleCase(sharedSkills[0])}` });

  if (sharedAvailability) {
    factors.push({ weight: detailScores.availability, label: `Similar availability: ${titleCase(sharedAvailability)}` });
  } else if (detailScores.availability >= 0.6) {
    factors.push({ weight: detailScores.availability, label: "Similar availability" });
  }

  if (sharedLanguages[0]) factors.push({ weight: detailScores.languages, label: `Shared language: ${titleCase(sharedLanguages[0])}` });
  if (firstNeighbourhood && firstNeighbourhood === secondNeighbourhood) {
    factors.push({ weight: detailScores.neighbourhood, label: `Same neighbourhood: ${titleCase(firstNeighbourhood)}` });
  }

  if (factors.length < 4 && detailScores.hours >= 0.85) {
    factors.push({ weight: detailScores.hours, label: "Similar time available each month" });
  }
...
function computeVolunteerSimilarity(currentVolunteer, otherVolunteer) {
  ...
  const languageScore = sharedNonEnglishLanguages.length
    ? computeArrayOverlapScore(
        normalizeMultiValueField(currentVolunteer.languages_spoken).filter((language) => language !== "english"),
        normalizeMultiValueField(otherVolunteer.languages_spoken).filter((language) => language !== "english")
      )
    : 0;

  const detailScores = {
    causeAreas: computeArrayOverlapScore(currentVolunteer.cause_areas_of_interest, otherVolunteer.cause_areas_of_interest),
    skills: computeArrayOverlapScore(currentVolunteer.skills, otherVolunteer.skills),
    availability: computeAvailabilitySimilarity(currentVolunteer.availability, otherVolunteer.availability),
    languages: languageScore,
    neighbourhood:
      toComparableString(currentVolunteer.neighbourhood) &&
      toComparableString(currentVolunteer.neighbourhood) === toComparableString(otherVolunteer.neighbourhood)
        ? 1
        : 0,
    hours: computeNumericCloseness(currentVolunteer.hours_available_per_month, otherVolunteer.hours_available_per_month, 40),
    ...
    backgroundCheck: computeBackgroundStatusSimilarity(currentVolunteer.background_check_status, otherVolunteer.background_check_status),
...
function findTopSimilarVolunteers(currentVolunteer, volunteers, limit = 3) {
